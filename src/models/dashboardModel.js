...
function buscarTemperaturasTanque(idUsuario, idTanque) {
     console.log(
        "ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function buscarTemperaturasTanque(): ",
        idUsuario,
        idTanque,
    );

    var instrucaoSql = `
        SELECT r.idRegistro, r.temperatura, r.dtHora
        FROM registro r 
        JOIN sensor s ON s.idSensor = r.fkSensor 
        JOIN tanque t ON t.idTanque = s.fkTanque
        JOIN vinicola v ON v.idVinicola = t.fkVinicola
        JOIN usuario u ON u.idUsuario = v.fkUsuario
        WHERE u.idUsuario = ${idUsuario} AND t.idTanque = ${idTanque}
...
function buscarTanques(idUsuario) {
  console.log(
    "ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function buscarTanques(): ",
    idUsuario,
  );

  var instrucaoSql = `
    SELECT t.idTanque, t.codigoTanque FROM tanque t
    JOIN vinicola v ON v.idVinicola = t.fkVinicola
    JOIN usuario u ON u.idUsuario = v.fkUsuario
    WHERE u.idUsuario = ${idUsuario};`;
